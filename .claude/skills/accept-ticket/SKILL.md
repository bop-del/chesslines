...
a hand, or whether the feature is worth having. `CLAUDE.md` is blunt
...
`open "http://localhost:8000/?b1"` — and **ask Boris to confirm the number he
...
transport row at all: the server was serving the right build correctly, the check above
...
- **Truth.** Is what the app says about the position actually correct? This is
  the highest-stakes test in the walk and the one with no automatable
  equivalent. A trainer that calls a good move wrong teaches something false,
  authoritatively, to someone with no reason to doubt it. Check the line
  against a real source if there is any doubt at all.
- **Is it teaching?** Would Felix understand *why* the move is right from what
  is on screen, or only *that* it is? A trainer that marks answers without
  explaining them is a quiz, not a teacher. This is the test most likely to
  send work back, and the one a passing check says nothing about.
- **Touch on a real phone.** The iOS defenses in `css/board.css` are
  structurally invisible to the verification run — `touch-action` does not
  even apply on desktop Safari. Long-press a piece: no "Save Image" sheet.
  Drag a finger across the board: the page must not scroll. This needs the
  actual device, not the simulator.
...
|---|---|
| Copy, a CSS value, a constant, a wrong label | Touches the rules engine or the repertoire data |
