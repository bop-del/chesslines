...
The repo is `bop-del/chesslines`; `gh` infers it from `git remote -v` when run inside the clone.
...
| **Needs review** | Built and the checks pass. Waiting on the parts an agent cannot judge: whether it teaches, and the screenshot. | `/accept-ticket` |
...
- **Only Boris moves anything to `Done`.** Done means a human used it and looked
...
The board is project **3** under the `bop-del` **user** (not the repo) —
<https://github.com/users/bop-del/projects/3>. It needs the `project` token
scope; if a call 403s, run `gh auth refresh -h github.com -s project`.

```bash
# See the board — what is in flight, and what is free to take
gh project item-list 3 --owner bop-del --format json

# Add an issue
gh project item-add 3 --owner bop-del --url https://github.com/bop-del/chesslines/issues/<n>

# Move a card: find its item id in the list above, then
gh project item-edit --id <item id> \
  --project-id PVT_kwHODNi2i84Bifsp \
  --field-id PVTSSF_lAHODNi2i84BifspzhhXkZI \
...
1. Snapshot first — `gh project item-list 3 --owner bop-del --format json` and
